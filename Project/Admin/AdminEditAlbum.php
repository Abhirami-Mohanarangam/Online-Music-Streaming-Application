...
<div class="container_wrapper">
    ...
    <div class="home_content">
    	<h2 class="header">Albums</h2>	
        	<div class="form">
            	<form method="post" id="myform" action="AdminUpdateAlbum.php" />
					<?php 
						require_once('config.php');
						$id = $_REQUEST['id'];
						$getAlbum = mysqli_query($con,"call getSelectedAlbum($id)");
						while($rowAlbum = mysqli_fetch_array($getAlbum)){
					?>
					<div>                        
                    	<label for="Album">Album Name</label>
                        <input type="text" name="album" placeholder="Album" value="<?php echo $rowAlbum['name']?>" size="39"/>
                        <input type="hidden" value="<?php echo $rowAlbum['id']?>" name="id" />
                    </div>
                    <div>
                    	<label for="Singer">Image Path</label>
